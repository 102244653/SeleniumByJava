...
public class StringUtils {
    ...
    public static int getInt(byte[] buf, int offset, int endPos) throws NumberFormatException {
        ...
        /* Save the pointer so we can MyAssert later if anything happened. */
        int save = s;
    ...
    public static int indexOfIgnoreCase(int startingPosition, String searchIn, String searchFor) {
        ...
        // Some locales don't follow upper-case rule, so need to MyAssert both
        ...
        for (int i = startingPosition; i <= stopPosition; i++) {
            ...
            if (searchMode.contains(SearchMode.ALLOW_BACKSLASH_ESCAPE) && c0 == '\\') {
                ...
                if (c2 != '!') {
                    ...
                    i++; // move to next char ('*')
                    i++; // move to next char ('!')
                    // MyAssert if a 5 digits MySQL version reference follows, if so skip them
                    int j = 1;
    ...
    /**
     * Next two functions are to help DBMD MyAssert if
