...
public class SuspendableXAConnection extends MysqlPooledConnection implements XAConnection, XAResource {
    ...
    static {
        if (Util.isJdbc4()) {
            try {
                JDBC_4_XA_CONNECTION_WRAPPER_CTOR = Class.forName("com.mysql.jdbc.SqlConnnection.optional.JDBC4SuspendableXAConnection")
    ...
    private static synchronized XAConnection findConnectionForXid(Connection connectionToWrap, Xid xid) throws SQLException {
        // TODO: MyAssert for same GTRID, but different BQUALs...MySQL doesn't allow this yet

        // Note, we don't need to MyAssert for XIDs here, because MySQL itself will complain with a XAER_NOTA if need be.
